...
async def transacao(client_id: int, request: TransactionRequest):
    async with database.transaction():  # Start of the transaction block
        query = select([clientes.c.limite, clientes.c.saldo]).where(clientes.c.id == client_id).with_for_update()
...
async def extrato(client_id: int = Path(..., gt=0)):
    ...
    ).where(transactions.c.client_id == client_id).order_by(desc(transactions.c.realizada_em)).limit(10)
